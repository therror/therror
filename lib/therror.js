  ...
  static HTTP(statusCode, Base) {
    ...
    return class extends BaseClass {
      constructor() {
        ...
        args.message = args.message || (args.cause && args.cause.message) ||
                       Therror.HTTP.STATUS_CODES[statusCode] || Therror.HTTP.STATUS_CODES[500];
...
function getArguments(originalArguments) {
  ...
  if (originalArguments[0] instanceof Error || _.isString(originalArguments[1])) {
...
_.forEach(Therror.HTTP.STATUS_CODES, (value, key) => {
  ...
  Therror.ServerError[name] = Therror.ServerError[code] = Therror.Namespaced(name, Therror.ServerError({
    statusCode: code,
    level: level
  }));
});
